...
part 'add_register_appoinment_event.dart';
part 'add_register_appoinment_state.dart';
part 'add_register_appoinment_bloc.freezed.dart';

class AddRegisterAppoinmentBloc
    extends Bloc<AddRegisterAppoinmentEvent, AddRegisterAppoinmentState> {
  AddRegisterAppoinmentBloc() : super(_Initial()) {
    on<AddRegisterAppoinmentEvent>((event, emit) async {
      try {
        if (event is _Started) {
          emit(const _Loading());
          final response = await ApiService.apiCalling(
              body: event.reqBody, apiName: Api.registerAppoinment);
